...
def main():
    ...
    parser.add_argument('--dataset_root', type=str, default = 'datasets/custom/custom_preprocessed', help='dataset root dir (''YCB_Video_Dataset'' or ''Linemod_preprocessed'')')
    parser.add_argument('--workers', type=int, default = 2, help='number of data loading workers')
    parser.add_argument('--iteration', type=int, default = 2, help='number of refinement iterations')
    parser.add_argument('--model', type=str, default = 'ckpt/v3/last.ckpt',  help='PoseNet model')
    parser.add_argument('--refine_model', type=str, default = 'ckpt/df-epoch=51-val_loss=0.00409.ckpt',  help='PoseRefineNet model')
    ...
    if opt.model != '':
        ...
        estimator.cuda()
        s_dict = {}
        sdict = torch.load(opt.model)['state_dict']
        for key in sdict:
            if key[0] == 'e':
                key1 = key[10:]
                s_dict[key1] = sdict[key]
        torch.save(s_dict, 'estimator.pth')
        estimator.load_state_dict(torch.load('estimator.pth'))

    if opt.refine_model != '':
        refiner = PoseRefineNet(num_points = opt.num_points, num_obj = opt.num_objects)
        refiner.cuda()
        s_dict = {}
        sdict = torch.load(opt.refine_model)['state_dict']
        for key in sdict:
            if key[0] == 'r':
                key1 = key[8:]
                s_dict[key1] = sdict[key]
        torch.save(s_dict,'refine.pth')
        refiner.load_state_dict(torch.load('refine.pth'))
